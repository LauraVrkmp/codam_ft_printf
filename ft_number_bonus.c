...
/*   Updated: 2025/11/23 05:24:53 by laveerka      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static int	write_length(char *num_str, t_flags *type)
{
	...
	if (type->precision >= num_length && type->period)
		while (type->width > (type->plus + type->space + type->precision + type->neg) && !type->minus)
			written += spacing_zeros(&type->width, ' ');
	else
		while (type->width > (num_length + type->plus + type->space) && (!type->zero || (type->zero && type->precision >= 0)) && !type->minus)
			written += spacing_zeros(&type->width, ' ');
	...
	if (type->neg)
		written += write(1, "-", 1);
	while (type->width > (num_length + type->plus + type->space) && type->zero && type->precision == -1 && !type->minus)
		written += spacing_zeros(&type->width, '0');
	while (type->precision > (num_length - type->neg + type->space + type->plus))
	...
	if (prec > num_length)
		while (type->width > (prec + type->plus + type->space + type->neg) && type->minus)
			written += spacing_zeros(&type->width, ' ');
	else
		while (type->width > (num_length + type->plus + type->space) && type->minus)
			written += spacing_zeros(&type->width, ' ');
...
t_flags *type)
{
	...
	if (num_str == NULL)
		return (0);
	while (type->period && type->precision <= 0 && num_str[0] == '0' && type->width > 0)
